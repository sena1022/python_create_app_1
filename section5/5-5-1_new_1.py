...
try:
    with sqlite3.connect('C:/Django/workspace/python-class1/section5/database/sqlite.db') as conn: #DB생성(파일)
        ...
        gs = fdr.DataReader('090430', start, end) #아모레퍼시픽(KRX : 090430) 주가 읽기
        ...
        gs.to_sql("TEST", conn, if_exists="replace", index=True, index_label='Id') #fail(table 無), replace(table 有일시, 삭제후 다시.. 신중히!!), append
